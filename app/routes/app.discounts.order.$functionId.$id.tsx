...
export const shouldRevalidate: ShouldRevalidateFunction = ({ json, defaultShouldRevalidate }) => {
  console.log('check revalidate');
  if(json) {
    return JSON.parse(json?.toString())?.errors?.length === 0
  }
...
export default function OrderDiscountApp() {
  ...
  const success = search.get("success")?.toLocaleLowerCase() === "true"
  ? true
  : !search.has('success')
  ? null
  : false;
  const result =  useActionData<{ discount?: DiscountInput, errors: ErrorBannerProps["errors"]}>();
  const data = useLoaderData<{ id?: string, discount?: DiscountInput, functionId: string, errors?: ErrorBannerProps["errors"]}>();
  ...
  const errors: ErrorBannerProps["errors"] = [
    ...submitErrors,
    ...(result?.errors ?? []),
    ...(data?.errors ?? []),
    ];
