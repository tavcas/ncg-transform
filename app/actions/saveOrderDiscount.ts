...
export default async function saveOrderDiscount({ params, request }: DataFunctionArgs){
    ...
    const metafields = [
      {
        id: configuration.id,
        namespace: "paymentplan-discount",
        key: "value",
        type: "json",
    ...
    if (DiscountMethod[method] === DiscountMethod.Code) {
    ...
    : [CODE_DISCOUNT_UPDATE, {
      ...
      if(errors.length > 0) {
        throw json({ errors, discount: { ...baseCodeDiscount, ...discount, functionId, metafields } });
      }
      const redirectUrl = discount.appDiscountType.appBridge.detailsPath.replaceAll(":functionId", functionId).replaceAll(":id", discount.discountId.split("/").pop()) + "?success=true";
    ...
    : [AUTOMATIC_DISCOUNT_UPDATE, {
      ...
      if(errors.length > 0) {
        throw json({ errors, discount: { ...baseDiscount, ...discount, functionId, metafields } });
      }
      const redirectUrl = discount.appDiscountType.appBridge.detailsPath.replaceAll(":functionId", functionId).replaceAll(":id", discount.discountId.split("/").pop()) + "?success=true";
      return redirect(redirectUrl)
    }
