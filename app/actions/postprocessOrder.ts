...
import { ADD_DISCOUNT_MUTATION, ADD_VARIANT_MUTATION, BEGIN_EDIT_MUTATION, EDIT_QUANTITY_MUTATION } from "../graphql";
import type { Mutation } from "../graphql/generated/graphql";
import { shopifyId } from "../lib/shopify";
import { json } from "react-router";
import { ProductDetails } from "../types";

type OrderAddVariantResponse = Exclude<Mutation['orderEditAddVariant'], null | undefined>;
...
const paymentPlanFromNotes = (note_attributes: any[]) => {
    const { value: paymentPlan } = note_attributes.find(n => n.name ==='Payment Plan');
    if(!paymentPlan) {
        return new Response(
            "Order ignored cause it doesn't have a payment plan", 
            {
                status: 304
            }
        )
...
const discountFromPaymentPlan = (paymentPlan: string) => {
    ...
    if(discount_percentage < 0) {
        return new Response(
            "No order line needs a price adjustment",
            {
                status: 304
            }
        )
...
const orderDetailsFromNotes = (note_attributes: any[]) => {
    const orderDetails: string = note_attributes.find(n => n.name === 'Order details')?.value;
    if(!orderDetails) {
        throw new Response(
            "Order doesn't have a order details",
            {
                status: 400
            }
        )
...
const primaryProductFromNotes = (note_attributes: any[]) => {
    const primaryProduct: string = note_attributes.find(n => n.name === 'Primary Product Details')?.value;
    if(!primaryProduct) {
        throw new Response(
            "Order doesn't have a primary product",
            {
                status: 400
            }
        )
...
const summarizeQuantities = (orderDetails: OrderDetailNote[]) => {
    ...
    const quantitiesByVariant: KeyNumberMap = orderDetails.reduce((p, c) => ({ ...p, [c.variant]: p[c.variant] ? p[c.variant] + c.quantity: c.quantity}), {});
    ...
    const discountsByVariant: KeyNumberMap = lines.reduce(
        (p, l) => 
            ({
                ...
                ? p[String(l.variant_id)] + l.discount_allocations.reduce((s, d) => s + Number(d.value), 0)
...
export default async function postProcessOrder({ request }: DataFunctionArgs) { 
  ...
  async function graphql<T extends MaybeUserError>(query: string, variables: any): Promise<T> {
  ...
  const orders = await request.json() as OrderCreatePayload[]
  for(const order of orders) {
    ...
    //begin edit 
    const response = await admin.graphql(BEGIN_EDIT_MUTATION, { variables: { orderId: shopifyId(orderId, 'Order') }});
    if(!response.ok) {
        throw response;
    }

    const beginData = await graphql<Mutation['orderEditBegin']>(BEGIN_EDIT_MUTATION, { orderId: shopifyId(orderId, 'Order') })
    const lines = beginData?.calculatedOrder?.lineItems?.edges.map(({ node }) => node);
    ...
    for(const { id } of lines ?? []) {
        await graphql<Mutation['orderEditSetQuantity']>(EDIT_QUANTITY_MUTATION, { id: calculatedOrderId, lineitemId: id, quantity: 0 });
    }

    for( const [variant, quantity] of Object.entries(quantitiesByVariant)) {
        const { calculatedLineItem: { id: lineItemId }} = await graphql<OrderAddVariantResponse>(ADD_VARIANT_MUTATION, { id: calculatedOrderId, variantId: shopifyId(Number(variant), 'ProductVariant'), quantity }).then(r => r ?? {});
        await graphql<OrderSetQuantity>(ADD_DISCOUNT_MUTATION, { discount_percentage, id: calculatedOrderId, lineItemId, discountDescription: 'Payment Plan' });
        await graphql<OrderSetQuantity>(ADD_DISCOUNT_MUTATION, { discount_percentage, id: calculatedOrderId, lineItemId, discountDescription: 'Payment Plan' });
        ...
        if(variant === String(primaryProduct.variant)) {
            for(const { code: description , amount, type } of discount_codes) {
                const discount = type === "fixed_amount" 
                ? {  description, fixedValue: amount }
                : { description, percentValue: amount };
                await graphql<OrderSetQuantity>(ADD_DISCOUNT_MUTATION, { discount, id: calculatedOrderId, lineItemId });
            }
        }
